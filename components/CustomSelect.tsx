...
function CustomSelect<T extends string | number>({ 
  ...
  useLayoutEffect(() => {
    if (isOpen && listRef.current && selectedRef.current) {
      ...
      // We assume the list container has 'relative' positioning so offsetTop is relative to it
  ...
  const styles = variant === 'default' 
    ...
    : {
        // Adjusted padding (py-1.5) and added fixed height (h-8) for better vertical alignment
